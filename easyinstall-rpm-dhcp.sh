...
read -p "Enter default lease time in seconds (e.g., 86400): " DEFAULT_LEASE
read -p "Enter max lease time in seconds (e.g., 604800): " MAX_LEASE
read -p "Enter gateway/router IP (e.g., 192.168.200.1): " ROUTER
read -p "Enter your domain name (e.g., mydomain.local): " DOMAIN_NAME
read -p "Enter your DNS server 1 (e.g., 192.168.200.1 or 8.8.8.8): " DNS_SERVER
read -p "Enter your DNS server 2 (e.g., 192.168.200.1 or 8.8.4.4): " DNS_SERVER2
...
echogreen "To start/stop the service:"
echogreen "    systemctl start|stop dhcpd"
